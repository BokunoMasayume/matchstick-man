...
function attachRotate(obj , ancestors){
    ...
    obj.group.addEventListener("mousemove", function(e){
        ...
        preV = m3.vMulMat(preV  , mat);
        nowV = m3.vMulMat(nowV , mat);
        preV[0] *= scaleX;
        nowV[0] *= scaleX;
        preV[1] *= scaleY;
        nowV[1] *= scaleY;
...
function attachTranslate(obj , ancestors){
    ...
    obj.group.addEventListener("mousemove",function(e){
        ...
        let scaleMat = calcScale(obj , ancestors);
        tv = m3.vMulMat(tv , mat);
        tv[0] = tv[0]*scaleMat[0][0];
        tv[1] = tv[1]*scaleMat[1][1];
...
function calcScale(obj,ancestors){
    let mat = m3.unit();
    ancestors.forEach(anc=>{
        // console.log("mul:" ,anc.mat )
        mat = m3.multipy(mat , anc.scale);
        // console.log("--res:", mat);
        if(anc.tmpTrans){
            // deprecated ,when tmpTrans is Array
            // anc.tmpTrans.forEach(tran=>{
